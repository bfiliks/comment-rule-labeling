...
with st.sidebar.expander("📝 Annotator Instructions", expanded=True):
    ...
    2. Use the **search bar** to find comments by keywords.
    3. For each pair:
       - Assign a label:  
         `0` → **Not Violating**  
         `1` → **Violates Rule**
...
# --- Search Bar ---
search_term = st.text_input("🔍 Search by keyword in rule or comment:")
if search_term:
    filtered = df[
        df["rule_text"].str.contains(search_term, case=False, na=False) |
...
if filtered.empty:
    st.success("🎉 All comments have been labeled or no match found for search.")
    st.stop()
